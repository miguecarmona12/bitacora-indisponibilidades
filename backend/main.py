...
try:
    with engine.connect() as conn:
        # Añade empresa_id a aplicaciones temporalmente si no existe (SQLite)
        # Migramos si había data anterior en empresa_id a la nueva tabla ManytoMany
...
try:
    with engine.begin() as conn:
        conn.execute(text("ALTER TABLE aplicaciones ADD COLUMN empresa_id INTEGER REFERENCES empresas(id)"))
except Exception:
    pass
...
# === RUTAS PARA APLICACIONES (Nuevo Nivel Medio) ===
...
def create_aplicacion(aplicacion: schemas.AplicacionCreate, db: Session = Depends(get_db), current_user: models.Usuario = Depends(auth.get_current_active_user)):
    if current_user.rol != 'admin':
        raise HTTPException(status_code=403, detail="No autorizado")
    db_aplicacion = db.query(models.Aplicacion).filter(models.Aplicacion.nombre == aplicacion.nombre).first()
    if db_aplicacion:
    ...
    if aplicacion.empresa_ids:
        empresas_vinculadas = db.query(models.Empresa).filter(models.Empresa.id.in_(aplicacion.empresa_ids)).all()
...
def update_aplicacion(aplicacion_id: int, aplicacion_update: schemas.AplicacionUpdate, db: Session = Depends(get_db), current_user: models.Usuario = Depends(auth.get_current_active_user)):
    ...
    if aplicacion_update.empresa_ids is not None:
        ...
        if aplicacion_update.empresa_ids:
            empresas_vinculadas = db.query(models.Empresa).filter(models.Empresa.id.in_(aplicacion_update.empresa_ids)).all()
...
# === RUTAS PARA CATEGORÍAS (Nuevo Nivel Superior a Producto) ===
...
# === RUTAS PARA PRODUCTOS (Antiguo 'Servicio') ===
...
def create_producto(producto: schemas.ProductoCreate, db: Session = Depends(get_db), current_user: models.Usuario = Depends(auth.get_current_active_user)):
    ...
    nuevo_producto = models.Producto(nombre=producto.nombre, categoria_id=producto.categoria_id)
...
def get_incidentes(db: Session = Depends(get_db), mes: str = None, current_user: models.Usuario = Depends(auth.get_current_active_user)):
    ...
    if current_user.rol == 'cliente':
        query = query.filter(models.Incidente.empresa_id == current_user.empresa_id)
    if mes:
...
def create_incidente(incidente: schemas.IncidenteCreate, db: Session = Depends(get_db), current_user: models.Usuario = Depends(auth.get_current_active_user)):
    if current_user.rol == 'cliente':
        raise HTTPException(status_code=403, detail="Los clientes no pueden crear incidentes")
...
def delete_incidente(incidente_id: int, db: Session = Depends(get_db), current_user: models.Usuario = Depends(auth.get_current_active_user)):
    if current_user.rol != 'admin':
        raise HTTPException(status_code=403, detail="Sin permisos para eliminar incidentes")
...
def update_incidente(incidente_id: int, incidente_update: schemas.IncidenteUpdate, db: Session = Depends(get_db), current_user: models.Usuario = Depends(auth.get_current_active_user)):
    if current_user.rol not in ['admin', 'tecnico']:
        raise HTTPException(status_code=403, detail="Sin permisos para editar incidentes")
        
    db_incidente = db.query(models.Incidente).filter(models.Incidente.id == incidente_id).first()
    if not db_incidente:
        raise HTTPException(status_code=404, detail="Incidente no encontrado")
